...
module DiscourseTemplates::TopicQueryExtension
  def list_category_templates
    ...
    list = default_results(options.reverse_merge(unordered: true, no_definitions: true))
      .references(:category)
      .includes(:first_post)
      .includes(:template_item_usage)
      .where("topics.category_id IN (?)", all_templates_categories) # filter only topics in the configured categories and subs
      .where(
        visible: true,
        archived: false
  ...
  def list_private_templates
    ...
    list = list
      ...
      .joins(:tags).where("tags.name in (?)", private_template_tags)
      .reorder("topics.title ASC")
